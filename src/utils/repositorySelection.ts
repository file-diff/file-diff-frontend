...
export const CREATE_TASK_RUNNER_VALUES = ["codex", "claude", "opencode"] as const;
...
export async function requestCreateTag(
  repo: string,
  tag: string,
  commit: string,
  bearerToken: string,
  ...
  const response = await fetch(CREATE_TAG_API_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${bearerToken}`,
    },
    body: JSON.stringify({ repo, tag, commit }),
    signal,
  });

  if (!response.ok) {
    let message = `Unable to create tag "${tag}"`;
...
export async function requestDeleteActionRun(
  repo: string,
  runId: number,
  bearerToken: string,
  signal?: AbortSignal
): Promise<DeleteActionRunResponse> {
  const response = await fetch(DELETE_ACTION_RUN_API_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${bearerToken}`,
  ...
  if (!response.ok) {
    let message = `Unable to delete workflow run "${String(runId)}"`;
  ...
  return (await response.json()) as DeleteActionRunResponse;
}
